import {
  Column,
  Entity,
  Index,
  JoinColumn,
  ManyToOne,
  OneToMany,
  OneToOne,
  PrimaryGeneratedColumn,
} from "typeorm";
import { ProductCommentLikes } from "./ProductCommentLikes";
import { Users } from "./Users";
import { Products } from "./Products";
import { Subproducts } from "./Subproducts";
import { ProductRate } from "./ProductRate";
import { WalletGiftCharges } from "./WalletGiftCharges";

@Index("product_comments_approved_by_index", ["approvedBy"], {})
@Index("product_comments_parent_comment_id_index", ["parentCommentId"], {})
@Index("product_comments_product_id_index", ["productId"], {})
@Index("product_comments_subproduct_id_index", ["subproductId"], {})
@Index("product_comments_user_id_index", ["userId"], {})
@Entity("product_comments", { schema: "mydatabase" })
export class ProductComments {
  @PrimaryGeneratedColumn({ type: "int", name: "id", unsigned: true })
  id: number;

  @Column("int", { name: "parent_comment_id", nullable: true, unsigned: true })
  parentCommentId: number | null;

  @Column("int", { name: "product_id", unsigned: true })
  productId: number;

  @Column("int", { name: "subproduct_id", nullable: true, unsigned: true })
  subproductId: number | null;

  @Column("int", { name: "user_id", unsigned: true })
  userId: number;

  @Column("int", { name: "approved_by", nullable: true, unsigned: true })
  approvedBy: number | null;

  @Column("longtext", { name: "comment" })
  comment: string;

  @Column("tinyint", {
    name: "approved",
    comment: "1=approved,2=trashed,-1=starred,unapproved=0",
    width: 1,
    default: () => "'0'",
  })
  approved: boolean;

  @Column("tinyint", { name: "starred", width: 1, default: () => "'0'" })
  starred: boolean;

  @Column("tinyint", { name: "is_buyer", nullable: true, width: 1 })
  isBuyer: boolean | null;

  @Column("tinyint", { name: "recommended", nullable: true, width: 1 })
  recommended: boolean | null;

  @Column("int", { name: "total_likes", default: () => "'0'" })
  totalLikes: number;

  @Column("int", { name: "total_dislikes", default: () => "'0'" })
  totalDislikes: number;

  @Column("timestamp", { name: "created_at", nullable: true })
  createdAt: Date | null;

  @Column("timestamp", { name: "updated_at", nullable: true })
  updatedAt: Date | null;

  @OneToMany(
    () => ProductCommentLikes,
    (productCommentLikes) => productCommentLikes.productComment
  )
  productCommentLikes: ProductCommentLikes[];

  @ManyToOne(() => Users, (users) => users.productComments, {
    onDelete: "NO ACTION",
    onUpdate: "CASCADE",
  })
  @JoinColumn([{ name: "approved_by", referencedColumnName: "id" }])
  approvedBy2: Users;

  @ManyToOne(
    () => ProductComments,
    (productComments) => productComments.productComments,
    { onDelete: "NO ACTION", onUpdate: "CASCADE" }
  )
  @JoinColumn([{ name: "parent_comment_id", referencedColumnName: "id" }])
  parentComment: ProductComments;

  @OneToMany(
    () => ProductComments,
    (productComments) => productComments.parentComment
  )
  productComments: ProductComments[];

  @ManyToOne(() => Products, (products) => products.productComments, {
    onDelete: "CASCADE",
    onUpdate: "CASCADE",
  })
  @JoinColumn([{ name: "product_id", referencedColumnName: "id" }])
  product: Products;

  @ManyToOne(() => Subproducts, (subproducts) => subproducts.productComments, {
    onDelete: "CASCADE",
    onUpdate: "CASCADE",
  })
  @JoinColumn([{ name: "subproduct_id", referencedColumnName: "id" }])
  subproduct: Subproducts;

  @ManyToOne(() => Users, (users) => users.productComments2, {
    onDelete: "NO ACTION",
    onUpdate: "CASCADE",
  })
  @JoinColumn([{ name: "user_id", referencedColumnName: "id" }])
  user: Users;

  @OneToMany(() => ProductRate, (productRate) => productRate.productComment)
  productRates: ProductRate[];

  @OneToOne(
    () => WalletGiftCharges,
    (walletGiftCharges) => walletGiftCharges.productComment
  )
  walletGiftCharges: WalletGiftCharges;
}
